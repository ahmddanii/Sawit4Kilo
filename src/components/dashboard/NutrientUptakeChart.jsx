...
// Dummy data: last 7 days of TDS changes. Negative means nutrient uptake (good), Positive means concentration increase (evaporation/stress)
const data = [
  { day: 'Sen', delta: -120 },
  { day: 'Sel', delta: -85 },
  { day: 'Rab', delta: -140 },
  ...
  return (
    <div className="bg-white rounded-lg p-3 border border-[#EAECF0] shadow-sm flex flex-col gap-1 min-w-[120px]">
      ...
      <span className="text-[10px] text-[#98A2B3]">
        {isPositive ? 'Konsentrasi naik (Penguapan)' : 'Penyerapan nutrisi baik'}
      </span>
    </div>
  );
};

const NutrientUptakeChart = () => {
  return (
    <div className="bg-white rounded-[12px] border border-[#EAECF0] p-5 flex flex-col h-full">
      <div className="mb-4">
        <h3 className="text-[14px] font-bold text-[#101828] leading-none">
          Laju Penyerapan Nutrisi
        </h3>
        <p className="text-[12px] text-[#98A2B3] mt-1.5 leading-tight">
          Delta TDS Harian (7 Hari Terakhir)
        </p>
      </div>
...
export default NutrientUptakeChart;
