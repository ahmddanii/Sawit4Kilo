...
const CompactStatusBlock = memo(({
  ...
  // ── pH status ──────────────────────────────────────────────
  const phStatus = phVal < phThresholdMin
    ? 'danger'
  ...
  const tdsStatus = tdsVal > TDS_LIMIT
    ? 'danger'
    : tdsVal > TDS_WARN
    ? 'warning'
    : 'stable';
    ...
    <div className="flex flex-col gap-3 h-full">
      ...
      <div className={`flex items-center justify-between px-3.5 py-2 rounded-xl border ${
        ...
        <div className="flex items-center gap-2">
          <span className={`h-1.5 w-1.5 rounded-full ${sysCfg.dot} animate-pulse`} />
          <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Status Sistem</span>
        </div>
        <span className={`text-[11px] font-black uppercase tracking-widest ${
          systemStatus === 'BAHAYA' ? 'text-red-700' : 'text-emerald-700'
        }`}>
          {systemStatus === 'BAHAYA' ? 'BAHAYA' : 'NORMAL'}
        </span>
      </div>
          ...
          <div>
            ...
            {isLoading ? (
              ...
              <div className="flex items-baseline gap-1 mt-0.5">
                <span className="font-mono font-bold text-[38px] tracking-tight text-slate-900 leading-none">
                  {phVal.toFixed(1)}
                </span>
      ...
      <div className="bg-white rounded-xl border border-slate-200/50 shadow-sm p-4 flex flex-col gap-3 flex-1">
        ...
        <div>
          ...
          <div className="relative h-1.5 w-full bg-slate-100 rounded-full">
            {/* Safe zone */}
            <div
              className="absolute top-0 left-0 h-full bg-emerald-200/70 rounded-full"
